// src/components/TabelaTipos.jsx
import React from "react";
import { Link } from "react-router-dom";

const TabelaTipos = ({ dados, loading, integracao }) => {
  if (loading) {
    return (
    ...
    return (
      <div className="overflow-x-auto bg-white shadow rounded-lg p-4 dark:bg-gray-800">
        <p className="text-gray-500 dark:text-gray-400">
          {integracao
            ? `Nenhum tipo de anúncio encontrado para a integração '${integracao}'.`
          ...
          {dados.map((item) => (
            ...
            >
              ...
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                {/* ✅ Link para a próxima página da perfuração */}
                <Link
                  to={`/analise/integracoes/${encodeURIComponent(
                    integracao
                  )}/tipos/${encodeURIComponent(item.TIPO_ANUNCIO)}/skus`}
